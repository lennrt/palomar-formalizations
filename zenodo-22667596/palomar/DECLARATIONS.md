...
The root `comparator.json` selects all 42 results below for one Palomar entry.
The table links their original proofs; 29 selections use those original names,
while 13 use definitionally identical `PalomarVerified` interfaces from
[PalomarWrappers.lean](PalomarWrappers.lean). The complete exact original-to-selected
mapping is in [selection-map.json](verification/selection-map.json); no result
was removed. Every selected proof is supplied by `Solution`; all proof source is in this
...
| [`ZombieMain.diamond_ring_sensor_bound`](ZombieMain/SensorCounting.lean#L9) | Attributed ring sensor cardinality lower bound |
...
| [`ZombieDamage.Graph.involution_blocks_recovery`](ZombieDamage/Observation.lean#L74) | Involution collision obstructs observation recovery |
| [`ZombieDamage.DiamondRing.central_pair_sensor`](ZombieDamage/DiamondRing.lean#L97) | Every central diamond pair needs a sensor; attributed supporting result |
...
`Challenge` independently states all 42 results with their necessary definitions;
...
only Std. Its additional comparison is retained by the local verification script.
...
- `central_pair_sensor`, `metric_exit_contract`, `metric_target_contract`.
