...
- [All 42 independent statements and their definitions](Challenge.lean).
...
real Landrun sandbox, and runs both the 42-declaration unified comparison and
the original 24-declaration independent game-statement comparison. Local logs
...
type-alias issue. The [selection map](verification/selection-map.json) retains
all 42 results; the other 29 names are unchanged. No mathematical statement is
weakened. The final axiom audit checks all 263 supplied roots plus these 13 wrappers.
The deliberate statement holes are confined to `Challenge.lean` (all 42
...
formalized. Computational census counts and historical regressions are tests,
not extra selected theorem claims.

The statement surface exceeds Palomar's preferred 300 lines/32 KiB, because
it exposes all 42 results and actual game definitions, but is within the hard
1,000-line/100-KiB limits (973 lines, 53,034 bytes). See [VERIFICATION.md](VERIFICATION.md) for the exact
checked size and current renderer result. This advisory is not a size-limit
failure; editorial review remains Palomar's decision.
